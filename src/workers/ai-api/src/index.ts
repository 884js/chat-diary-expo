...
import { GoogleGenerativeAI, type GenerationConfig, SchemaType, type Schema } from '@google/generative-ai';
import type { Env, OGPResponse, AIEmotionResponse, AIChatResponse } from '../types/index.js';
import {
  DEFAULT_CONFIG,
  ERROR_CODES,
  createCacheKey,
...
// 感情判定用のAI関数
async function analyzeEmotion(message: string, apiKey: string): Promise<AIEmotionResponse> {
...
// AI応答生成関数
async function generateAIResponse(message: string, emotion: string, apiKey: string): Promise<AIChatResponse> {
  const genAI = createAIClient(apiKey);
...
現在のユーザーの感情: ${emotion}
感情に応じた応答の開始フレーズ: ${emotionContext[emotion as keyof typeof emotionContext]}`;
...
app.post('/api/ogp', async (c: Context<{ Bindings: Env }>) => {
  ...
  try {
    ...
    c.executionCtx.waitUntil(
      c.env.AI_CACHE.put(cacheKey, JSON.stringify(ogpData), {
...
// AI感情判定エンドポイント
app.post('/api/ai-emotion', async (c: Context<{ Bindings: Env }>) => {
  const clientIP = c.req.header('CF-Connecting-IP') || 'unknown';
  ...
  try {
    ...
    if (!message || typeof message !== 'string') {
      return c.json(
        { error: 'Message is required', code: ERROR_CODES.INVALID_URL },
        400,
      );
    }
    ...
    if (!c.env.GEMINI_API_KEY) {
      return c.json(
        { error: 'AI service unavailable', code: ERROR_CODES.INTERNAL_ERROR },
        500,
      );
    }
...
// AI応答生成エンドポイント
app.post('/api/ai-chat', async (c: Context<{ Bindings: Env }>) => {
  const clientIP = c.req.header('CF-Connecting-IP') || 'unknown';
...
// 感情判定 + AI応答の統合エンドポイント
app.post('/api/ai-message', async (c: Context<{ Bindings: Env }>) => {
  const clientIP = c.req.header('CF-Connecting-IP') || 'unknown';
...
// AI問いかけ生成関数
async function generatePrompt(type: 'morning' | 'afternoon' | 'evening' | 'emotion-based', emotion?: string, apiKey?: string): Promise<string> {
  if (!apiKey) {
    // APIキーがない場合のフォールバック
    const prompts = {
...
// AI問いかけエンドポイント
app.post('/api/ai-prompt', async (c: Context<{ Bindings: Env }>) => {
  const clientIP = c.req.header('CF-Connecting-IP') || 'unknown';
  ...
  try {
    ...
    if (!type || !['morning', 'afternoon', 'evening', 'emotion-based'].includes(type)) {
      return c.json(
        { error: 'Invalid prompt type', code: ERROR_CODES.INVALID_URL },
        400,
      );
    }
